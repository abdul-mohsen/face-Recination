...
for folder in os.listdir(args["dataset"]):
	try:
		...
		os.mkdir("datasetDone/" + folder)
	except FileExistsError:
		pass

# moving images to Done dir
for (i, imagePath) in enumerate(imagePaths):
	try:
		os.rename(imagePath,"datasetDone\\" + imagePath[8:])
	except:
		os.rename(imagePath,"datasetDone\\0" + imagePath[8:])
# read old data
data_old = {"embeddings":[],"names":[]}
try:
	data_old = pickle.loads(open(args["embeddings"], "rb").read())
except :
	print("Directory " , args["embeddings"] ,  " does not exists")
data = {"embeddings": data_old["embeddings"] + knownEmbeddings, "names": data_old["names"] + knownNames}
